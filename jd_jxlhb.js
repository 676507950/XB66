...
[task_local]
#京喜领88元红包
5 0,12,21 * * * https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jd_jxlhb.js, tag=京喜领88元红包, img-url=https://raw.githubusercontent.com/Orz-3/mini/master/Color/jd.png, enabled=true

==============Loon==============
[Script]
cron "5 0,12,21 * * *" script-path=https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jd_jxlhb.js,tag=京喜领88元红包

================Surge===============
京喜领88元红包 = type=cron,cronexp="5 0,12,21 * * *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jd_jxlhb.js

===============小火箭==========
京喜领88元红包 = type=cron,script-path=https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jd_jxlhb.js, cronexpr="5 0,12,21 * * *", timeout=3600, enable=true
 */
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    ...
    if ($.canHelp && ($.authorMyShareIds && $.authorMyShareIds.length)) {
      ...
      for (let j = 0; j < $.authorMyShareIds.length && $.canHelp; j++) {
        console.log(`【${$.UserName}】去助力作者的邀请码：${$.authorMyShareIds[j].strUserPin}`);
        $.max = false;
...
function getJxToken() {
  function randomPhoneid(length) {
      let srt = "abcdefghijklmnopqrstuvwxyz1234567890";
      let ss = '';
      for (let i = 0;i < length; i++) {
        ss += srt[ parseInt(Math.random() * srt.length) ];
      }
      return ss;
  }
  return new Promise(resolve => {
      let phoneid = randomPhoneid(0x28);
      let timestamp = (+new Date()).toString();
      if (!cookie.match(/pt_pin=([^; ]+)(?=;?)/)) {
          console.log(`此账号cookie填写不规范,你的pt_pin=xxx后面没分号(;)\n`);
          resolve();
      }
      let pin = cookie.match(/pt_pin=([^; ]+)(?=;?)/)[0x1];
      resolve({
          'timestamp': timestamp,
          'phoneid': phoneid,
          'farm_jstoken': $.md5('' + decodeURIComponent(pin) + timestamp + phoneid + 'tPOamqCuk9NLgVPAljUyIHcPRmKlVxDy').toString()
      });
  });
};
